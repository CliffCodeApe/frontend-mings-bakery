<script lang="ts">
  ...
  async function fetchOrders() {
    ...
    if (!token) return;
    const res = await fetch('https://mings-bakery-production.up.railway.app/api/orders/user', {
...
async function checkout() {
  ...
  const res = await fetch('https://mings-bakery-production.up.railway.app/api/orders/make', {
    method: 'POST',
    headers: {
